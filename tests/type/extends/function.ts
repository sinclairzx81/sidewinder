...
describe('type/extends/Function', () => {
  ...
  it('Should extend Function 1', () => {
    ...
    const R = Extends.Check(Type.Function([], Type.Number()), Type.Function([], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Function 2', () => {
    type T = (() => any) extends () => number ? 1 : 2
    const R = Extends.Check(Type.Function([], Type.Any()), Type.Function([], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.True)
  })
  ...
  it('Should extend Function 4', () => {
    ...
    const R = Extends.Check(Type.Function([Type.Number()], Type.Number()), Type.Function([], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Function 5', () => {
    type T = ((a: number | string) => number) extends (a: number) => number ? 1 : 2
    const R = Extends.Check(Type.Function([Type.Union([Type.Number(), Type.String()])], Type.Number()), Type.Function([Type.Number()], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Function 6', () => {
    type T = ((a: number) => number) extends (a: number | string) => any ? 1 : 2
    const R = Extends.Check(Type.Function([Type.Number()], Type.Number()), Type.Function([Type.Union([Type.Number(), Type.String()])], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Function 7', () => {
    type T = ((a: number, b: number) => number) extends (a: number) => number ? 1 : 2
    const R = Extends.Check(Type.Function([Type.Number(), Type.Number()], Type.Number()), Type.Function([Type.Number()], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Function 8', () => {
    type T = ((a: number) => number) extends (a: number, b: number) => number ? 1 : 2
    const R = Extends.Check(Type.Function([Type.Number()], Type.Number()), Type.Function([Type.Number(), Type.Number()], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Function 9', () => {
    type T = (() => number) extends () => any ? 1 : 2
    const R = Extends.Check(Type.Function([], Type.Number()), Type.Function([], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.True)
  })
  ...
  it('Should extend Function 12', () => {
    ...
    const R = Extends.Check(Type.Function([], Type.Object({})), Type.Function([], Type.Array(Type.Any())))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Function 13', () => {
    type T = ((a: unknown) => number) extends (a: any) => number ? 1 : 2
    const R = Extends.Check(Type.Function([Type.Unknown()], Type.Number({})), Type.Function([Type.Any()], Type.Number({})))
    Assert.deepEqual(R, ExtendsResult.True)
  })
