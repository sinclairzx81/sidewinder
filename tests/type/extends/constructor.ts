...
describe('type/extends/Constructor', () => {
  it('Should extend Function', () => {
    ...
    const R = Extends.Check(Type.Constructor([], Type.Number()), Type.Function([], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.False)
  })
  it('Should extend Constructor 1', () => {
    type T = (new () => number) extends new () => number ? 1 : 2
    const R = Extends.Check(Type.Constructor([], Type.Number()), Type.Constructor([], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Constructor 2', () => {
    type T = (new () => any) extends new () => number ? 1 : 2
    const R = Extends.Check(Type.Constructor([], Type.Any()), Type.Constructor([], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.True)
  })
  ...
  it('Should extend Constructor 4', () => {
    ...
    const R = Extends.Check(Type.Constructor([Type.Number()], Type.Number()), Type.Constructor([], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Constructor 5', () => {
    type T = (new (a: number | string) => number) extends new (a: number) => number ? 1 : 2
    const R = Extends.Check(Type.Constructor([Type.Union([Type.Number(), Type.String()])], Type.Number()), Type.Constructor([Type.Number()], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Constructor 6', () => {
    type T = (new (a: number) => number) extends new (a: number | string) => any ? 1 : 2
    const R = Extends.Check(Type.Constructor([Type.Number()], Type.Number()), Type.Constructor([Type.Union([Type.Number(), Type.String()])], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Constructor 7', () => {
    type T = (new (a: number, b: number) => number) extends new (a: number) => number ? 1 : 2
    const R = Extends.Check(Type.Constructor([Type.Number(), Type.Number()], Type.Number()), Type.Constructor([Type.Number()], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.False)
  })

  it('Should extend Constructor 8', () => {
    type T = (new (a: number) => number) extends new (a: number, b: number) => number ? 1 : 2
    const R = Extends.Check(Type.Constructor([Type.Number()], Type.Number()), Type.Constructor([Type.Number(), Type.Number()], Type.Number()))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Constructor 9', () => {
    type T = (new () => number) extends new () => any ? 1 : 2
    const R = Extends.Check(Type.Constructor([], Type.Number()), Type.Constructor([], Type.Any()))
    Assert.deepEqual(R, ExtendsResult.True)
  })
  ...
  it('Should extend Constructor 10', () => {
    ...
    const R = Extends.Check(Type.Constructor([], Type.Array(Type.Any())), Type.Constructor([], Type.Object({})))
    Assert.deepEqual(R, ExtendsResult.True)
  })
  ...
  it('Should extend Constructor 13', () => {
    ...
    const R = Extends.Check(Type.Constructor([Type.Unknown()], Type.Number({})), Type.Constructor([Type.Any()], Type.Number({})))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Constructor 14', () => {
    type T = (new (a: any) => number) extends new (a: unknown) => number ? 1 : 2
    const R = Extends.Check(Type.Constructor([Type.Any()], Type.Number({})), Type.Constructor([Type.Unknown()], Type.Number({})))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Constructor 15', () => {
    type T = (new () => any) extends new () => unknown ? 1 : 2
    const R = Extends.Check(Type.Constructor([], Type.Any({})), Type.Constructor([], Type.Unknown({})))
    Assert.deepEqual(R, ExtendsResult.True)
  })

  it('Should extend Constructor 16', () => {
    type T = (new () => unknown) extends new () => any ? 1 : 2
    const R = Extends.Check(Type.Constructor([], Type.Unknown({})), Type.Constructor([], Type.Any({})))
    Assert.deepEqual(R, ExtendsResult.True)
  })
